...
Create new data frame with numerical steps and the watt generation. This is done to avoid date handling
```{r}
...
```{r}
get_artificial_list <- function(mean_real_yr, gam_prediciton) {
  ...
  while (treshold) {
       ...
       n <- (n/50)
       base_yr_with_noise <- gam_prediciton * n
       k <- (mean(df_plantA_resample_2$Generation_kW) / mean(base_yr_with_noise))
    
    treshold <- (k > 1.05 | k < 1)
    print(i)
    print(k)
    print(mean(base_yr_with_noise))
    i = i + 1
    base_yr_with_noise
...
```{r}
plot(get_artificial_list(mean(df_plantA_resample_2$Generation_kW), gam_prediciton))
```
...
while (treshold) {
  ...
  treshold <- (k > 1.2 | k < 1.1)
  print(i)
  print(k)
  print(mean(base_yr_with_noise))
  i = i + 1
}
plot(base_yr_with_noise)
```
```{r}
k <- 0.9
treshold <- (k > 1.2 | k < 1)
